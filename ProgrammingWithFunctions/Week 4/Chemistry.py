...
def parse_formula(formula, periodic_table_dict):
    ...
    symbol_quantity = defaultdict(int)
    matches = re.findall(r'([A-Z][a-z]?)(\d*)', formula)
    for symbol, quantity in matches:
        symbol_quantity[symbol] += int(quantity) if quantity else 1
    return list(symbol_quantity.items())
